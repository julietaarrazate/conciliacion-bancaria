"""Servicio de reportes y analisis ejecutivo.

Toda la logica de agregacion/calculo que antes vivia en `routers/analisis.py`:
dashboard de KPIs, alertas operativas, aging de clientes, estado de cuenta por
cliente, evolucion mensual y flujo de caja. El router solo se ocupa de
decoradores HTTP, query params, permisos y cache de transporte; este modulo
hace las queries y arma los dicts de respuesta.
...
STATUS_OK = {"ok", "OK", "PAGO_PARCIAL", "CONCILIADO_CON_DIFERENCIA"}
...
def _kpis_periodo(db: Session, org_id: int, desde: date, hasta: date) -> dict:
    ...
    conciliado = _suma_planilla_rows(db, org_id, desde, hasta, STATUS_OK, usar_fecha_acred=True)
    # Pendiente y total: por fecha_carga (planillas cargadas en el período)
    pendiente = _suma_planilla_rows(db, org_id, desde, hasta, STATUS_PENDIENTE)
...
def _top_clientes_mes(db: Session, org_id: int, desde: date, hasta: date, limit: int = 5) -> list:
    ...
    rows = (
        ...
        .filter(
            Planilla.organizacion_id == org_id,
            Planilla.deleted_at.is_(None),
            PlanillaRow.status.in_(list(STATUS_OK)),
            func.date(Planilla.fecha_carga) >= desde,
            func.date(Planilla.fecha_carga) <= hasta,
        )
...
def calcular_rango(periodo: str, anio: Optional[int], mes: Optional[int],
    ...
    a = anio or hoy.year
    m = mes or hoy.month
    desde, hasta = rango_mes(a, m)
    if m == 1:
        prev_desde, prev_hasta = rango_mes(a - 1, 12)
    else:
        prev_desde, prev_hasta = rango_mes(a, m - 1)
...
def calcular_dashboard(
    ...
    desde_r, hasta_r, prev_desde, prev_hasta, label = calcular_rango(periodo, anio, mes, desde, hasta)
...
def calcular_alertas(db: Session, organizacion_id: int) -> dict:
    ...
    filas_atrasadas = (
        ...
        .filter(
            ...
            Planilla.deleted_at.is_(None),
            PlanillaRow.status.in_(list(STATUS_PENDIENTE)),
...
def calcular_clientes_aging(db: Session, organizacion_id: int, limit: int, offset: int) -> dict:
...
def calcular_estado_cuenta_cliente(
    db: Session,
    cliente,
    desde: Optional[date],
    hasta: Optional[date],
    ...
    `cliente` ya viene resuelto y validado (existencia + org) por el router.
    """
    ...
    for p in planillas:
        ...
        sub_pendiente = 0
        for row in p.rows:
            estado_norm = "ok" if row.status in STATUS_OK else (
                "pendiente" if row.status in STATUS_PENDIENTE else (row.status or "otro")
            )
            if estado_norm == "ok":
...
def calcular_evolucion(db: Session, organizacion_id: int, meses: int) -> dict:
    ...
    for a, m in periodos:
        desde, hasta = rango_mes(a, m)
        conc = _suma_planilla_rows(db, organizacion_id, desde, hasta, STATUS_OK)
        pend = _suma_planilla_rows(db, organizacion_id, desde, hasta, STATUS_PENDIENTE)
...
def calcular_flujo_caja(db: Session, organizacion_id: int, meses: int) -> dict:
