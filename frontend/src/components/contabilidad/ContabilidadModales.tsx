...
export const ExportContableModal: React.FC<{ c: ContabilidadCtx }> = ({ c }) => {
  if (!c.exportModalOpen) return null
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => c.setExportModalOpen(false)}>
      <div className="bg-white dark:bg-ml-dark-surface rounded-2xl shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-gray-200 dark:border-ml-dark-border flex items-center justify-between">
          <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Exportar Libro Diario</h3>
          <button onClick={() => c.setExportModalOpen(false)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">✕</button>
        </div>
        <div className="p-4 space-y-4">
          <div>
            ...
            >
              ...
              <option value="holistor">Holistor</option>
              <option value="regisoft">Regisoft</option>
            </select>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
              {c.exportFormato === 'csv' && 'CSV separado por comas, UTF-8, con header.'}
              {c.exportFormato === 'tango' && 'TXT tab-separado, coma decimal, Latin-1. Sin encabezado.'}
              {c.exportFormato === 'holistor' && 'TXT punto-y-coma, coma decimal, Latin-1.'}
              {c.exportFormato === 'regisoft' && 'CSV coma, punto decimal, UTF-8. Columnas: Fecha, Cuenta, Detalle, Debe, Haber.'}
            </p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Desde</label>
              <input type="date" value={c.exportDesde} onChange={e => c.setExportDesde(e.target.value)}
                className="w-full border border-gray-200 dark:border-slate-600 rounded-lg px-2 py-2 text-xs bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-300 mb-1">Hasta</label>
              <input type="date" value={c.exportHasta} onChange={e => c.setExportHasta(e.target.value)}
                className="w-full border border-gray-200 dark:border-slate-600 rounded-lg px-2 py-2 text-xs bg-white dark:bg-slate-700 text-gray-800 dark:text-gray-100" />
            </div>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500">
            Si no seleccionás fechas, se exporta todo el Libro Diario.
          </p>
          {c.exportWarn.length > 0 && (
            <div className="rounded-lg bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 p-3">
              <p className="text-xs font-semibold text-yellow-800 dark:text-yellow-300 mb-1">Cuentas sin mapeo configurado:</p>
              <p className="text-xs text-yellow-700 dark:text-yellow-400">{c.exportWarn.join(', ')}</p>
              <p className="text-xs text-yellow-600 dark:text-yellow-500 mt-1">El archivo se descargó con los códigos internos de Cuadra. Configurá el mapeo en Organizaciones si necesitás los códigos del software contable.</p>
            </div>
          )}
        </div>
        <div className="p-4 border-t border-gray-200 dark:border-ml-dark-border flex justify-end gap-2">
          <button onClick={() => c.setExportModalOpen(false)}
            className="text-xs px-3 py-2 rounded-lg border border-gray-300 dark:border-ml-dark-border text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-slate-700">
            Cancelar
          </button>
          <button onClick={c.handleExportContable} disabled={c.exportLoading}
            className="text-xs px-4 py-2 rounded-lg bg-ml-blue text-white font-medium hover:bg-ml-blue-dark disabled:opacity-50 flex items-center gap-1.5">
            {c.exportLoading ? (
              <>
