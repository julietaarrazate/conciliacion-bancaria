...
const navItems: Array<{ to: string; label: string; Icon: () => JSX.Element; permission?: string; superadmin?: boolean }> = [
  ...
  { to: '/tarjetas',      label: 'Tarjetas',    Icon: Icon.CreditCard, permission: 'manage_finance' },
  { to: '/pagos',         label: 'Pagos',       Icon: Icon.Bolt, permission: 'reconcile' },
...
export const Layout: React.FC = () => {
  ...
  const visibleItems = navItems.filter(
    item => {
      if (item.superadmin) return !!user?.is_superadmin
