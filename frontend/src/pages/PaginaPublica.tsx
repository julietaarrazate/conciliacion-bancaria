...
export const PaginaPublica: React.FC = () => {
  ...
  const totalConciliado = data.planillas.reduce((s: number, p: any) =>
    s + p.rows.filter((r: any) => r.status === 'ok' || r.status === 'OK').reduce((a: number, r: any) => a + r.monto, 0), 0)
  const totalPendiente = data.planillas.reduce((s: number, p: any) =>
    s + p.rows.filter((r: any) => !['ok', 'OK', 'duplicado'].includes(r.status)).reduce((a: number, r: any) => a + r.monto, 0), 0)
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {data.cheques.length > 0 && (
          <div className="space-y-2 mb-6">
            ...
            {data.cheques.map((c: any, i: number) => (
